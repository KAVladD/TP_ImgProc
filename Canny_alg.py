...
def canny_segment(clahe_img, cord, img1):
    ...
    for i in cord:
        outputCanny = cv2.Canny(bin_img[i[1]:i[1] + i[3], i[0]:i[0] + i[2]], 10, 150)
...
cord = find_rect(masked_first_frame)  # находим начальное положение областей интереса


frames = os.listdir(path_to_frames)

img1 = cv2.imread(path_to_frames + 'frame0.jpg')  # считываю первый кадр
clahe_img = prepare(img1)
dispers1 = np.var(img1[cord[0][1]:cord[0][1] + cord[0][3], cord[0][0]:cord[0][0] + cord[0][2]])  #дисперсия для первого кадра
dispers2 = np.var(img1[cord[1][1]:cord[1][1] + cord[1][3], cord[1][0]:cord[1][0] + cord[1][2]])  #дисперсия для второго кадра
...
for i in range(1, len(frames)):
    ...
    canny = canny_segment(clahe_img, cord, img1)
    cv2.imwrite(path_to_folder+ 'frame'+str(i)+'.jpg', canny)
